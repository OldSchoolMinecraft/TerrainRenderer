...
public class BlockColor {
	...
	// Static initializer to set up block colors and metadata usage
	static {
	...
	// Constructor to create BlockColor with ARGB values
	BlockColor(int argb) {
		this(argb, TintType.NONE);
	}

	// Constructor to create BlockColor with ARGB values and TintType
	BlockColor(int argb, TintType type) {
		...
		this.green = (float) (argb >> 8 & 255) / 255.0F;
		this.blue = (float) (argb >> 0 & 255) / 255.0F;
	}

	// Retrieve BlockColor based on id and metadata
	...
	// Check if metadata is used for the block id
	public static boolean useMetadata(int id) {
		return useMetadata[id];
	}

	// Calculate usage of metadata for blocks
	private static void calcUseMetadata() {
	...
	// Create an id based on block id and metadata
	private static int id(int id, int metadata) {
		return id << 4 | metadata;
	}

	// Create an id based on block name and metadata
	private static int id(String name, int metadata) {
		if (name == null) {
			return BLOCK_NUM << 4;
		} else {
			Integer id = nameMap.get(name);
			if (id == null && name.matches("#[0-9A-Fa-f][0-9A-Fa-f]")) {
				id = Integer.valueOf(Integer.parseInt(name.substring(1), 16));
			}
